...
## check_genes_in_pathway_03, Reactome_GSEA_exclude_MT_RPL_RPS_genes_04
## Filter out unwanted genes, filter(!grepl("^RPL", gene) & !grepl("^RPS", gene) & !grepl("^MT-", gene) & !grepl("^MRPL|^MRPS", gene))
...
output_excel_file <- file.path(rds_directory, "top15_genes_longcovid_control_all_cell_types_02.xlsx") # Updated file name
...
for (cell_type in cell_types_to_analyze) {
  ...
  rds_file <- file.path(rds_directory, paste0("pathway_enrichment_reactome_", cell_type, "_03.rds"))
