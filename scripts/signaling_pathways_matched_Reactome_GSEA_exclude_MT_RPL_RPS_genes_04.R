...
reactome_file <- "/home/cyang40/chingyao/long_covid_project/merged_samples_02/pathway_analysis_CT_Sub_combined_27celltypes/GSEA_analysis/Reactome_GSEA_exclude_MT_RPL_RPS_genes/top20_genes_all_cell_types_with_symbols_04_02.xlsx"
output_file <- "/home/cyang40/chingyao/long_covid_project/merged_samples_02/pathway_analysis_CT_Sub_combined_27celltypes/signaling_pathways_matched_Reactome_GSEA_exclude_MT_RPL_RPS_genes_04.xlsx"
...
for (sheet in sheet_names) {
  ...
  # Ensure "genes" column is split into lists
  ...
  for (i in 1:nrow(signaling_pathways)) {
    ...
    matched <- reactome_data %>%
      rowwise() %>%
      mutate(Matched_Genes = list(match_genes(signaling_genes, genes))) %>% # Find matched genes
